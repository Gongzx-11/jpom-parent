...
public class JpomManifest {
     ...
     * 当前运行类型
     */
    private Type type = DevOpsApplication.getAppType();
    /**
     * 端口号
     */
    private int port;
        ...
        if (jarFile.isFile()) {
            try (JarFile jarFile1 = new JarFile(jarFile)) {
                ...
                if (version != null) {
                    ...
                    String timeStamp = attributes.getValue("DevOps-Timestamp");
    ...
    public static File getRunPath() {
        ApplicationHome home = new ApplicationHome(DevOpsApplication.getAppClass());
    ...
    public static JsonMessage checkJpomJar(String path, Class clsName) {
        ...
        try (JarFile jarFile1 = new JarFile(jarFile)) {
            ...
            if (entry == null) {
                return new JsonMessage(405, "此包不是系统【" + DevOpsApplication.getAppType().name() + "】包");
            }
    ...
    public static void releaseJar(String path, String version) {
        ...
        String newFile = DevOpsApplication.getAppType().name() + "-" + version + FileUtil.JAR_FILE_EXT;
        ...
        //
        String typeName = DevOpsApplication.getAppType().name().toLowerCase();
        final String[] oldName = new String[]{typeName + ".log"};
        final boolean[] logBack = {true};
        FileUtil.readLines(getScriptFile(), DevOpsApplication.getCharset(), (LineHandler) line -> {
        ...
        FileUtil.writeLines(newData, getScriptFile(), DevOpsApplication.getCharset());
    }
    ...
    public static File getScriptFile() {
        ...
        String type = DevOpsApplication.getAppType().name();
