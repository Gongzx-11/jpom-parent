...
import gzx.devops.model.system.AgentAutoUser;
...
public class AgentAuthorize {
    ...
    private void checkPwd() {
        ...
        if (FileUtil.exist(path)) {
            ...
            try {
                ...
                if (StrUtil.isNotEmpty(oldAgentPwd)) {
                    ...
                    DefaultSystemLog.getLog().info("已有授权账号:{}  密码:{},授权信息保存位置：{}", this.agentName, this.agentPwd, FileUtil.getAbsolutePath(path));
                    return;
                }
        ...
        DefaultSystemLog.getLog().info("已经自动生成授权账号:{}  密码:{},授权信息保存位置：{}", this.agentName, this.agentPwd, FileUtil.getAbsolutePath(path));
    }
}
