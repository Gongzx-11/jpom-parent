...
import gzx.devops.controller.tomcat.TomcatOp;
...
//            todo  (strs[0].endsWith("jpomAgent")) {
            if (strs[0].endsWith("GZXTomcatAgent")) {
                continue;
            }
    ...
    private String tomcatCmd(TomcatInfoModel tomcatInfoModel, String cmd) {
        //todo String url = String.format("http://127.0.0.1:%d/jpomAgent/%s"
        String url = String.format("http://127.0.0.1:%d/GZXTomcatAgent/%s", tomcatInfoModel.getPort(), cmd);
        HttpRequest httpRequest = new HttpRequest(url);
        // 设置超时时间为3秒
        httpRequest.setConnectionTimeout(3000);
        String body = "";
